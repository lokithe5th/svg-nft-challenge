...
contract Sphere is ERC721, Ownable {
  ...
  function determineProperty(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        uint256 rand = bytes32ToUint(properties[tokenId], 1);
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string[16] memory parts;
        ...
        image = string(abi.encodePacked(image, parts[9], parts[10], parts[11], parts[12], parts[13], parts[14], parts[15]));

        string memory json = Base64.encode(
          bytes(string(abi.encodePacked(
            '{"name": "Planet #', toString(tokenId), '", "description": "We have stepped into a new galaxy. You know what to do. Remember: the Known are watching. The Planet is yours.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(image)), '"}'))));
